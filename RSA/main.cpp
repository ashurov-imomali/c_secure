...
std::string Decode(std::string fileName, int p, int q, int d){
    ...
    writeToFile(utf8str, "../output.txt");
    return utf8res;
}


std::string Encode(const std::string& fileName, int p, int q, int e) {
...
int main(){
    ...
    std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t>  convert;
    std::u32string file = convert.from_bytes(u32txt);
    int k = 0, counter = 0;
